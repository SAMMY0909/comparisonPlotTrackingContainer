...
input_file_lrt = "/home/sammy/eos/user/s/sgoswami/public/NN_SRC_DATA/ttbar/lrt/refined_lrt_tightuser.sgoswami.28415580._000200.output.h5"
...
with File(input_file_lrt, 'r') as h5file:
  ...
  l_tracks_lrt = tracks[ljets,:]
  c_tracks_lrt = tracks[cjets,:]
  b_tracks_lrt = tracks[bjets,:]

  rownum_l,colnum_l=l_tracks_lrt.shape
  rownum_c,colnum_c=c_tracks_lrt.shape
  rownum_b,colnum_b=b_tracks_lrt.shape
  
  print("The dimensions of the l array for lrt container is, after cuts: " +str(rownum_l)+" by "+str(colnum_l))
  print("The dimensions of the c array for lrt container is, after cuts: " +str(rownum_c)+" by "+str(colnum_c))
  print("The dimensions of the b array for lrt container is, after cuts: " +str(rownum_b)+" by "+str(colnum_b))

os.makedirs('combLRT_new',exist_ok=True)
os.chdir('combLRT_new')

for vartype in variables:
  print("The vartype is: "+ vartype+ "\n")
  for var in variables[vartype]:
    print("The var is: "+ var +"\n")
    lrt_l = l_tracks_lrt[l_tracks_lrt["valid"] == 1][var]
    lrt_c = c_tracks_lrt[c_tracks_lrt["valid"] == 1][var]
    lrt_b = b_tracks_lrt[b_tracks_lrt["valid"] == 1][var]
    
    plt.clf()
    
    plt.hist(lrt_l, color = ['r'], bins = Bins[var], histtype='step', alpha=0.5, density=True, label="ttbar LRT+STD corr to l-jets")
    plt.hist(lrt_c, color = ['b'], bins = Bins[var], histtype='step', alpha=0.5, density=True, label="ttbar LRT+STD corr to c-jets")
    plt.hist(lrt_b, color = ['g'], bins = Bins[var], histtype='step', alpha=0.5, density=True, label="ttbar LRT+STD corr to b-jets")
    ...
    plt.savefig(var + '_combined_lrt.png')
